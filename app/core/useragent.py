...
| **不开 HTTP/2** | **实测过**（2026-09-16，本机对 B 站 `nav` 各 6 次、连接已预热取中位）：h1.1 **25.4ms** vs h2 **20.5ms** ⇒ 每次快约 5ms，对"每 60 秒发几个请求"的后台轮询没有可感知影响；而 h2 的两个主要好处（多路复用、头部压缩）我们**本来就用不上** —— 平台内刻意串行（防风机控）、连接已复用。代价却是加 `h2`（连带 hpack/hyperframe）依赖 + 重打后端产物 |
