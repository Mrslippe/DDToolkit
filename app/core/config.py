...
class Settings:
    ...
    # 但用户不会全天醒着」⇒ 在**用户自己指定**的本地时段里把动态流降下来。
    # 三条纪律：
    #   · **默认关闭**（不改变任何现有行为，由用户显式开启并指定时刻）；
    #   · **只降动态流**：T0 直播轮询保持 60s 不动 —— 日历的场次起止时间由 live 跳变推导，
    #     降它就会让场次时间变粗（用户 2026-09-16 明确选了"只降动态流"）；
    #   · 与 R24b 不冲突：那条否掉的是"**隐藏到托盘就降频**"（会拖慢推送时效），
    #     本项是"**用户显式声明我睡了**"。
    QUIET_HOURS_ENABLED: bool = False            # 默认关（可热更）
    QUIET_HOURS_START: int = 3                   # 静默开始（本地时间整点 0-23；可热更）
    QUIET_HOURS_END: int = 9                     # 静默结束（可热更）；END <= START 时按**跨午夜**算
    QUIET_HOURS_DYNAMICS_MIN_SECONDS: int = 900  # 静默期动态流间隔下限（秒，默认 15 分钟；可热更）
