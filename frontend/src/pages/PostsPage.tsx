      ...
      >
        ...
        {scene.view === 'list' && (
          <>
            ...
            {/* 帖子无限滚动区：仅承载帖子流；grid key 用替换型变化指纹——
                追加 page>1 不触动 key，旧卡片不重挂重播动画 */}
            <div className="list-scroll" ref={listScrollRef}>
              <div className="list-inner">
                {error ? (
                  ...
                  <div
                    className={`post-grid${loading ? ' is-refetching' : ''}`}
                    /* 替换型指纹 key：筛选/账号/fetch-idle 变化时整格重挂重播入场；
                       无限追加只置尾，不改变指纹 */
                    key={`${accountKey}|${typeFilter ?? ''}|${searchKw}|${dateFrom}|${dateTo}|${refreshTick}`}
                  >
                    {posts.map((p, i) => (
