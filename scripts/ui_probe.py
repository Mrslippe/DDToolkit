...
def main() -> int:
    ...
    be_env = {
        **os.environ,
        ...
        # CORS（S1，devlog/201）：**改了默认值之后必须显式给探针开这一条**。
        #
        # 背景：`CORS_ORIGINS` 的默认值从 `"*"` 改成了空 —— 因为 `*` 等于让**任意网页**
        # 都能读本机后端（端口可扫，而 `.env` 里是活凭据）。Tauri 自身不受影响
        # （webview 直连 `127.0.0.1`，不走预检），但**探针走的是浏览器**：
        # 页面在 `localhost:<vite_port>`、后端在 `127.0.0.1:<be_port>` ——
        # 端口不同 + 主机名不同 ⇒ **跨源** ⇒ 没有允许头时浏览器**不让页面读响应**。
        #
        # 实测症状极具误导性：后端日志里**一条 401 都没有**（请求被浏览器拦在"读"那一步），
        # 页面上所有数据都是空的 ⇒ 探针报"缺投稿 chip / 缺 list-video 帧 / 页面标题为空"，
        # 看起来像**内容或布局**坏了。判据是 `git stash` 掉本批改动后探针全绿。
        #
