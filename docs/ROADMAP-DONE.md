...
> 用户当日一次性提了 10 条（§0 收集区的第一批）。除 **R9 只落地了"撤出展示"**（展示入口
> 待「账号信息历史快照」那条线，条目仍留在 `docs/TODO.md` §0），其余全部落地。
...
| R2 | 直播日历好像在定时动态抓取任务完成后会闪动，排查原因 | 修复 | 中 | ✅ devlog/066：根因 = `loading` 一来就把 42 格整块换成加载态，而每轮 `fetch-idle`（动态流 ~80s）都会触发 reload → 网格消失重建（趋势卡同款，还白扔 ECharts 初始化）。改为**只在无数据时**显示加载态 + 有旧数据时刷新失败给红字小注。**可选第二步**（`fetch-idle` 带 kind）仍在 `TODO` §1.1 |
