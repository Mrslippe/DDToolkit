...
### 7.1 升级/回退相关的三个位置（批次 16，devlog/207）

发版之后用户可能来问"升级出问题了怎么办"，先看这三处（**都在数据目录里**，
跟着数据目录一起走）：

| 位置 | 是什么 | 怎么用 |
|---|---|---|
| `<DATA_DIR>/backups/vtuber-<head>-<时间戳>.db` | **真跑 schema 迁移之前**自动复制的一份库（带 `-wal`） | 退版本回退用：把新版关掉，用它替换 `vtuber.db`（连 `-wal` 一起），再用**旧版**打开。保留最近 3 份；确认新版本没问题后可自行删除 |
| `<DATA_DIR>/vtuber.db.failed-<时间戳>` | 迁移失败时被隔离的坏库（连同 `-wal`） | 出现它说明**本次升级的迁移失败过**，应用当时已用空库起来。**不要删**：那是原始档案，配合诊断包定位后可恢复 |
| 「导出诊断」（横幅或设置页） | 纯文本诊断包：版本 / OS / 迁移结局 / 库形态 / 备份清单 / 占用 / 三个日志尾部 | 让用户整份发过来。**里面不含 `.env`、cookie 与会话 token**（有判据钉着） |

**用户能回退到什么程度**：装回旧版即可（安装包会覆盖 `binaries/backend`，数据目录不动）。
⚠️ **旧版打不开"被新版迁过的库"**（schema 更新了、旧版不认识）—— 所以回退的正确姿势是
**连备份一起回退**（上表第一行）；单独装回旧版只会让旧版在迁移那一步失败，
而今天的兜底会**把库隔离成 `.failed-*` 再用空库启动**（档案还在，但界面上是空的）。
