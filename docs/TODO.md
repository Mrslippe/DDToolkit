...
| **R22** | 2026-09-16 | 数据放在 C 盘会不会挤爆：**A** 图片缓存加上限 + LRU 淘汰 · **B** 占用可见（关于页）+ 数据目录可迁移到其他盘 · **D** 库维护（删数据真的还盘）· C = `posts.raw_json` 瘦身（**用户口径：暂缓**） | 用户知道谁在占地方、能自己决定数据放哪 | 高 | 🔄 **进行中**：**A + D 已落地**（devlog/103）→ **B 待做**（跨 Rust/Python/前端三层，单独一批） |
