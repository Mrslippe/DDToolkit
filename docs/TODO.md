...
| 门禁 | 命令 | 基线 |
|---|---|---|
| 后端 | `python -m pytest -q` | **434 passed**（R11 检索 16 + API 5；发布脚本 28；管线复盘 15；未登录能力 24；R13 路由契约 +1；R12a 风控字段 +1；R14a 运行时设置 +24；R14b 偏好 +5；**R18 关闭语义偏好 +1**；**R20 文案扫描（禁 `**` 与反引号）+1 / prefs 白名单 +1**；含 1 条真实网络冒烟，离线环境会 skip） |
