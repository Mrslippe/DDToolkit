...
| R10 | 2026-09-13 | 动态抓取名单应是**库里所有 V 的所有平台账号**按平台分组，名单之间并行、**名单内部串行**（例：B 站名单 3 个、微博名单 2 个），频率仍标定一分钟 | 修 R6 的理解偏差：R6 只抓"每 V 主账号"，微博账号因此从未进过 60s 轮 | 高 | ✅ **已落地**（devlog/078）：`_dynamics_lanes()` 取代主账号口径；名单内间隔自适应摊平（`gap=clamp((50s−N×1.5s)/N, 2s, 5s)`，短名单命中 5s 上限）；`DYNAMICS_CONCURRENCY` 默认 1（保留为紧急开关）；微博未登录时整条名单跳过。实测（7 个 V / 10 个账号）：一轮 40~44s、请求 `{bilibili: 8, weibo: 2}`、0 风控、周期 60~70s |
