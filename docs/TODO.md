...
| **R26** | 2026-09-16 | **请求头与设备指纹卫生**（起因：用户问"头信息是硬编码写死的，程序被其他人用了会不会更容易被风控"）：① **设备号 cookie 名写错** —— 代码发的是 `bvuid3=`，而 B 站 web API 读的是 `buvid3`（匿名实测 `x/frontend/finger/spi` 下发 `b_3`/`b_4`，与本机 `.env` 里那个 46 字符值同形）⇒ 疑似**等于没带设备指纹**；② 补齐真浏览器会发的 `sec-ch-ua` / `Sec-Fetch-*`，去掉脚本痕迹 `Connection: keep-alive`，评估开 HTTP/2；③ UA 永久冻结在 Edge 150；④ `runner.py` 的机器人 UA 版本停在 `0.5.2` | 去掉跨用户完全一致的"工具签名"；每个安装有自己的设备身份 | 中 | 📋 **待评估**（2026-09-16 由我盘点后提出）。⚠️ **硬约束**：设备号**必须按安装生成并持久化** —— 把一份值写进代码 = 全网用户共享设备身份，比"没有设备号"更糟。⚠️ ① 是**需真机 A/B 才能钉死的推断**（正确名 vs 现在的名字，看动态流那个敏感端点） |
| **R27** | 2026-09-16 | **风控冷却要"记得住、会升级"**：触发风控的冷却窗口目前**只在内存**（重启 / 应用内更新后立刻恢复满速）、固定 10 分钟不升级、解禁后也没有减速恢复期 | 被限流后不会"重启继续敲"；连续撞墙时自动拉开间隔 | 高 | 📋 **待评估**（我建议的 Q2-A）。实测依据：`scheduler._rate_limit_until` 是模块级变量、不落库 ⇒ 重启即遗忘；`RATE_LIMIT_COOLDOWN=600` 是常量、不随重复命中升级 |
...
> R24 托盘资源（117）。**本表当前剩 R25–R29（均未落地）**：R25 已按用户口径往后放，
> 优先做 **R26–R29** 这组请求侧 / 风控侧加固（盘点依据与实测数字见
> `docs/ARCHITECTURE.md` §3.5 的两行缺口与末尾那条注，以及 devlog/124）。
