...
| R15 | 2026-09-15 | 一些前端改造 | 顶栏左侧标题改为粗体、list视图搜索栏右侧筛选按钮文字布局中、card视图的平台徽标添加按钮在鼠标不hover的时候自动隐藏并且也不占位，让徽章紧贴下方分割线 | 中/低/高（依次） | **已受理** → §1.1 |
...
| **R15 前端三处小改** | 体验 / 快赢 | ① 顶栏标题 `font-weight: 400 → 700`（**字距同步调**：用探针量出真实标题宽再定值，5px 在粗体下偏散）；② 筛选钮文字居中 —— **先量再改**：取"文字节点左右间隙"与"斜切 pill（`-10deg`）的视觉中心"，多半需要斜切补偿而不是再调 padding（9-13 已把 padding 改对称）；③ `.pill-add` 现在 `37×37` **在流里始终占位**（这就是徽章贴不到分割线的原因）→ 改**绝对定位悬浮**（`opacity:0 + pointer-events:none + scale(.94)`，hover 淡入），徽章始终紧贴分割线且 hover 不跳动。**验收**：③ 非 hover 时"徽章右缘→分割线"间距达标**且 add 钮不可命中**、hover 后可见可命中（探针断言）；② 文字左右间隙差 ≤1px（按视觉中心）；① 三档布局不变量 0；hero 签名基线同批更新 |
