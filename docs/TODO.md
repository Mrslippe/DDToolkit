...
>
> **R37 档案视图** —— 命名落点：`PostsPage.tsx` 光条 title 与视图枚举注释、`UI-MAP.md` / `GLOSSARY.md` / README 功能表
> （探针按**索引**点按钮，对文案免疫，但文档与注释要同步）。
> **数据素材全都有，缺的是"画布 + 卡片注册表"**：纪念日 = `vtubers.birthday` / `debut_date`；
> 优质投稿 = `posts` 的播放/点赞/转发/评论（`postStats` 已有排序）；大事记 = **`vtuber_events` 表（P7 就建了，一直没用）**；
> 时间线 = `posts` 按天聚合 + `account_stat_snapshots` 趋势；自定义 = 标题/正文/外链（新表或 JSON）。
> **四个要拍板的点**：① **网格引擎**：自研（CSS Grid + pointer 拖拽缩放，约 300–400 行，几何完全可控、探针可断言）
> vs 引入 `react-grid-layout`（成熟但重、样式难对齐设计语言、断言困难）→ 我倾向**自研**；
> ② **存储**：新表 `profile_cards(id, vtuber_id, kind, x, y, w, h, config_json, z)` + 迁移（与 R35 同一批）
> vs 在 `vtubers` 上挂一个布局 JSON → 我倾向**新表**（加卡片类型不用改布局存储）；
> ③ **扩展点**：`registerCardKind({id, name, minSize, defaultSize, render})`（照 `registerIdleProvider` 先例），
> 内置 4 种 + 用户自定义卡片；
> ④ **交互**：拖拽/缩放手柄、栅格吸附（如 12 列）、碰撞是"推开"还是"拒绝"、编辑态与阅读态分离、窄窗降级单列。
> 护栏：`utils/profileCards.ts` 纯函数单测（吸附 / 碰撞 / 序列化 / 默认布局）+ 新探针 `--profile-grid`
> （种 3 张卡片 → 断言网格几何、拖拽落格、缩放合法、**刷新后布局保持**（后端往返）、最小尺寸被强制）+ 反向验证。
> ⚠️ 它会给视图枚举**加第五个视图**，主探针的视图列表与部分基线要跟着扩。
> **建议分三批**：**P1** 命名 + 网格骨架 + 2 种卡片（纪念日 / 优质投稿，只读、默认布局）；
> **P2** 拖拽缩放 + 落库（新表 + 迁移）+ 编辑态；**P3** 扩展点 + 自定义卡片 + 大事记 / 时间线两种内置卡片。
...
| 前端单测 | `npm --prefix frontend run test` | **289 passed**（27 个文件；2026-09-17 实测，较上次记录 266 +23 —— **R31 场景机 `sceneStep` +11**、**R33 头像取值 `avatarSource` +6**、**R36 速览胶囊 `sessionGlance` +6**。分项：`addVtuberSearch` 7 · `capabilities` 6 · `dialogFoot` 3 · `format` 21 · `reservationDays` 9 · `notificationHub` 12 · `idleQuotes` 18 · `settingsDraft` 22 · `settingsNav` 16 · `theme` 13 · `shellState` 11 · `shellLifecycle` 6 · `trayStatus` 5 · **`sceneStep` 11** · **`avatarSource` 6** · **`sessionGlance` 6** · 其余既有。R35 的置顶渲染与 R36 的卡片渲染都没进这一层：本仓 vitest 跑 node 环境（无 jsdom），组件渲染由探针守（`--pinned` / `--archive` 的 R36 段） |
