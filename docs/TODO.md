...
| **R16 list 筛选钮跟随侧栏筛选钮** | ✅ **已落地**（2026-09-15，devlog/093） | 用户给两张截图（左栏「默认」= 参照，list「筛选」= 待改）。**先量后改**：新探针 `--filter-pill` 把两枚浮片逐项量化，量出真正的差异 —— 尺寸 89×25 vs 68×30 · 字 13 vs 12.5 · 内距 8 vs 12 · caret **absolute（右上 3px，不占流）** vs static · **文字中心偏移 0 vs −5.3px**。改法：`.pfilter-btn` 换成侧栏同一套配方（`min-width:89px` + 内距 8 + 字 13 + `line-height:1`）+ 去掉 `size="md"` + 删掉 R15 的 `pill-caret{position:static}` 覆盖 ⇒ 探针**逐项一致**（唯一豁免 `minWidth`：侧栏是定宽格子，list 用 min-width 以便「筛选 · N」能长）。<br>**探针逼出一个真问题**：合成的超长文案（「筛选 · 12 项」）下 caret 距文字只剩 **0.6px**（caret 落脚区 9.63px > 内距 8px）⇒ 文案外套 `.pf-label`（**左右各 11px 对称留白**）：文字仍是几何中心（三态偏移全 0），长文案让浮片自己长（89 → 106.6）、caret 与文字恒留 9.4px。短文案渲染与参照那枚完全一致。<br>**R15② 被取代**：那版把 caret 留在流内让"文字+箭头"居中，文字仍偏 5.3px；`--polish` 断言随之收紧为 **文字偏移 ≤1px + caret 必须 absolute**。视觉上另用 `--shot` 截图裁两枚并排比对过 |
