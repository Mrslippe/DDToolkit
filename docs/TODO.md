...
| **R37** | 2026-09-17 | **档案视图 = 自定义卡片画布**：把"档案卡"这一版做成**以卡片为单位**，用户能编辑卡片的大小 / 位置 / 排布，卡片内容可自定义（纪念日、优质投稿、大事记、时间线…），并**支持扩展** | 每个 V 的档案页由用户自己搭 | 中 | 🔨 **P1 已落地**（devlog/141）：命名改定（档案卡 → **档案视图**、档案 → **数据视图**）+ 只读卡片画布（12 列网格，`layoutModel.ts` **17 条单测**）+ 卡片注册表（`registerCardKind`，**4 条**）+ 两张内置卡片（纪念日 **14 条** / 优质投稿 **9 条**）。**四个选型已拍板**（自研网格 · 新表 `profile_cards` · 碰撞推开 + 编辑/阅读分离 + 窄窗单列 · 前端注册表）⇒ P2（拖拽缩放 + 落库，迁移 `f006`）与 P3（自定义卡片 + 扩展点）待做 |
...
| **R37-P2 档案视图：拖拽缩放 + 落库** | 前端 + 迁移，无阻塞（选型已定，见 §0） | 迁移 `f006` 建 `profile_cards`（`vtuber_id` 外键 + kind/x/y/w/h/config_json/z，**同步 `purge.py`**）+ 后端 CRUD 路由 + 前端拖拽/缩放（复用 `layoutModel` 的 `clampCard`/`normalizeLayout` 的**推开**口径）+ 编辑态（手柄 / 网格辅助线 / 重置默认）+ 落库失败回滚。护栏：几何单测扩拖拽用例 + 探针新增 `--board`（种布局 → 断言卡片位置与落库一致、越界被夹、窄窗单列） |
