...
| **R21** | 2026-09-16 | 设置窗口与同类二级弹窗改版（用户给了 4 张参考图）：① 左侧分类精简到 **4 项**（外观 / 抓取设置 / 数据源 / 关于）② 抓取设置**只留关键项**，并按用途分组（首屏抓取 / 开播信息抓取 / 定期动态轮询 / 每日定时任务…）③ **文字排版更醒目** ④ 数字输入框改成参考图二的整行 stepper ⑤ **所有二级独立弹窗右下角按钮改圆角浮片**（图三）⑥ 开关改成"稍大药丸内嵌滑块 + **去掉外框底色** + 浮片质感" | 没有专业背景的用户也能看懂每一项在调什么；视觉与项目浮片语言统一 | 高 | 🔄 **进行中**：**批 1（①②）已落地**（devlog/100）→ 批 2（③④⑥）/ 批 3（⑤）待做 |
...
| 门禁 | 命令 | 基线 |
|---|---|---|
| 后端 | `python -m pytest -q` | **437 passed**（R11 检索 16 + API 5；发布脚本 28；管线复盘 15；未登录能力 24；R13 路由契约 +1；R12a 风控字段 +1；R14a 运行时设置 +24；R14b 偏好 +5；**R18 关闭语义偏好 +1**；**R20 文案扫描（禁 `**` 与反引号）+1 / prefs 白名单 +1**；**R21 导航只有两大类 / 关键项白名单 / 成对不拆散 +3**；含 1 条真实网络冒烟，离线环境会 skip） |
