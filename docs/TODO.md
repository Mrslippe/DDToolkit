...
| **R21** | 2026-09-16 | 设置窗口与同类二级弹窗改版（用户给了 4 张参考图）：① 左侧分类精简到 **4 项**（外观 / 抓取设置 / 数据源 / 关于）② 抓取设置**只留关键项**，并按用途分组 ③ **文字排版更醒目** ④ 数字输入框改成参考图二的整行 stepper ⑤ **所有二级独立弹窗右下角按钮改圆角浮片** ⑥ 开关改成"稍大药丸内嵌滑块 + **去掉外框底色** + 浮片质感" | 没有专业背景的用户也能看懂每一项在调什么；视觉与项目浮片语言统一 | 高 | ✅ **已落地**（devlog **100 / 101 / 102**）：批 1 信息架构 · 批 2 排版与控件 · 批 3 弹窗页脚浮片 |
| **R22** | 2026-09-16 | 数据放在 C 盘会不会挤爆：**A** 图片缓存加上限 + LRU 淘汰 · **B** 占用可见（关于页）+ 数据目录可迁移到其他盘 · **D** 库维护（删数据真的还盘）· C = `posts.raw_json` 瘦身（**用户口径：暂缓**） | 用户知道谁在占地方、能自己决定数据放哪 | 高 | ✅ **已落地**：A+D（devlog/103）· B1 存储面板与清理（104）· B2a 指针与回退判定（105）· B2b 启动优先级与便携版判定（106）· B2c 迁移核心（107）· B2d 接线（108）+ 三次真机修复（109 脏目标目录 / 110 不复制 `-shm` / 111 verbatim 前缀）· B2e 壳侧日志与停后端加固（112）；**真机迁移已验证通过** |
| **R23** | 2026-09-16 | 应用内从 GitHub 获取更新包：签名密钥 · 更新源 · 发布链 `latest.json` · 关于页「检查更新」 · 启动静默检查 · 便携版不自我更新 · 代理兜底 | 用户不必手动下载安装包 | 高 | ✅ **已落地**：R23a 通道与发布链（113）· R23b 界面与启动检查（114）· R23c 更新检查的代理兜底（115）· R23d 错误分类（116）。⚠️ **端到端验证要等第一个 Release 建好**（此前"检查更新"必然报 remote 类失败——远端还没有 `latest.json`） |
| **R24** | 2026-09-16 | 托盘状态下减少资源占用 | 后台挂着时更省内存/电 | 中 | ✅ **已落地（按用户口径收窄）**：T1 看门狗按需唤醒（117）· 不再启动预热分词词典（117，**省约 56MB**）；**「隐藏后降频」不做** —— 见 R25 的联动口径 |
| **R25** | 2026-09-16 | **把消息推送到其他平台**（把"某 V 开播 / 发了新动态"这类事件推到外部渠道，让用户不必开着界面也能收到） | 后台常驻时也能第一时间收到通知 | 待定 | 📋 **待排期**（用户 2026-09-16 提出）。⚠️ **前提约束**：**后台/托盘挂起期间必须保持"尽可能快"的抓取节奏**——所以 R24 的"隐藏后降频"被**明确取消**：降频会直接拖慢推送时效。设计时要顺着这条走（见 `ARCHITECTURE.md` §3.10 第 4 条） |
