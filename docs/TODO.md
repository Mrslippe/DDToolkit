...
| R13 | 2026-09-15 | 动态中包括的直播预约记录到直播日历中 |  | 低 | **已受理** → §1.1 |
...
| **R13 直播预约进日历** | 数据已就绪（前端为主） | **链路已存在**：`fetcher._extract_reservation` → `posts.body_json.reservation` → `VtuberEventRepo.future_reservations()` → `GET /vtuber/{id}/future-reservations`；**缺**前端 api 封装与日历渲染（`ReservationInfo` 类型已存在）。方案：① 后端加聚合端点 `GET /reservations/upcoming?days=90`（一次拿全部已订阅 V，避免 N 请求，带 `vtuber_id/name`）；② 日历格加**预约标记** `.lc-resv`（虚线描边，**不动九类色系**），格内/hover 浮层显示「预约 21:00 标题」，当天有实场次时角标并存，图例加一项；③ 已过期预约与无预约**不显示任何标记**。**验收**：API 用例（含过期过滤）· 前端纯函数单测（按日期归格、与场次共存）· 探针断言预约格文本含「预约」· **日历签名基线同批重取** |
