...
> 区块里是 `.lc-dlg-ph` 的**「加载中…」「暂无动态数据」文案**，而左列封面下方**是空的**（截图红圈处）；
> 数据到达后 DOM 长高 ⇒ **弹窗高度跳变**。改法：① 左列封面下方加**信息胶囊卡**（`.lc-dlg-glance`）：
> 先用**已有数据**渲染（时长 / 收益 / 分区 / 数据源），上游回来再补（峰值在线 / 弹幕总量 / 打赏人数 / 互动）；
> 未回时用**同尺寸占位胶囊**（灰条 + 真实标签，而不是"加载中…"）；② 胶囊卡与右列卡片都定 `min-height`
> ⇒ 数据到达前后**高度差 = 0**；③ `.lc-dlg-ph` 的加载文案一并换成骨架（沿用"没有正在加载闪帧"的口径）。
> 护栏：扩 `ui_probe --archive`（或新 `--live-card`）**连采两格**（上游未回 / 已回）断言：红圈区域非空、
> 两态卡片高度差 0、占位胶囊与真实胶囊标签一一对应且**占位值不能长得像真值**（灰条 vs 数字）。
>
