...
| └ 词云 | `.lc-dlg-cloud` + svg | 高 **210px**；**圆形气泡簇**（2026-09-07 user 定案「从头开始」）：每个词 = 圆（`<circle>`），**半径恒定 ∝ √词频 → 面积 ∝ 词频（一以贯之：打开/破泡/恢复全程不变）**；半径计算 `computeRadii`（总盘面积 = 容器×0.72 泡沫密度；团簇直径 ≤ 0.92×容器高 → 圆形优先）；初始螺旋布置（大词在内、黄金角，确定性）→ `clusterSettle` 同步预求解（**打开即稳态，无开场动画**） |
| └ 气泡簇物理 | `clusterStep`（径向边界 + 重叠弹簧 + 洞吸力） | ①**径向边界** R = √(Σr²)（面积守恒圆半径），圆心 = 容器中心 → **整簇天然圆形**（用户条件②）；②边界软弹簧 KB=10 越界推回；③重叠软弹簧 KR=24（允许 10% 挤压 → 贴合处呈现「被邻泡压出的弧」，即交界不规则）；④速度阻尼 exp(−9·dt) + VMAX 420 + 容器 clamp；**破泡 = 去词（立即消失，无鼓泡/环波）→ 洞吸力（24 帧衰减 ×140px/s²，越近受力越大）+ R 收缩 → 周围泡泡立即闭合空洞**（用户条件③；node 原型调参：初始重叠 ≤8.1%、面积比例严格 ∝ 词频、闭合渐近收敛无 NaN、无极限环）；静止即停：帧位移总和 <1px 或 90 帧帽；reduced-motion = 同步结算瞬时闭合 |
...
| └ 词字显示 | — | `fs = max(8.5, min(hero?34:26, r*0.9, r*2.2/len))`；`r>10.5 && len<=6 && fs>=8.5` 才显示（放宽后「哈哈哈哈」不再被吞）；**渲染结构**：外层 `g.lc-dlg-cloud-cell`（`style transform: translate(x,y)`，模拟逐帧更新）+ `<circle r fill/stroke=--c-bg-card 2px>` + 居中 `<text>`（`pointer-events:none`）；`cursor:pointer` |
