...
- **四角白边（R33，devlog/135）**：`.app-shell` / `.splash` 只有 4px 圆角（`--radius-window`），圆角的抗锯齿像素会**透出 WebView 自己的背景** —— `transparent(true)` 只让**窗口**透明，WebView 层默认是**白的**，于是四角各留 1~2px 白边。修法在壳侧：`setup()` 调 `set_background_color(Some(Color(0,0,0,0)))`（该 API 同时设窗口与 WebView），深休眠唤醒重建窗口那条路用 builder 的 `.background_color(...)`。**改 CSS 是修不掉的**（不是页面画的）；DWM 已排除（关掉圆角/描边后像素值一字不变）
