...
**四个数决定"内容从哪开始"**（R45 / R45-B / R45-D，全仓唯一真源在 `tokens.css`）：
...
| `--toolbar-gap` | **40px** | **主体内容离它多远**（R45-D，见下） |

引用处只有两个（`.page-title` 与 `.hero`），都写 `calc(var(--toolbar-band) + var(--toolbar-gap))`。

> **`--toolbar-gap` 怎么来的**（R45-D，用户 2026-09-24）：用户给了两张参考图，要求
> 「参考这个比例，只参考比例其他设计元素均不要参考」，明确指的是
> 「**面板主体内容距离上面工具条的距离**」。用 PIL 逐行量参考图得到
> **37px / 占面板高 7.0%**（两张一致）⇒ 取 **40px**（占 581 面板高 6.9%）。
