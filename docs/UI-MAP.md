...
| 直播日历 | `<LiveCalendar>` `.live-calendar*` | **v0.8.0 重设计（照参考图周行列表）**：顶部统计行「共 N 场 + 类型彩色计数」（9 类标签：杂谈橙/游戏绿/观影蓝/投稿紫红/歌回红/健身黄/电台紫/联动青/特殊描边红）；主体=**周行列表**（每周一行 7 格，最新周在上、向下看历史，翻周 ◀▶ +「最近周」回锚）；格子 = 左上 `26/09/06 周六` + `[彩签] 时间` + 单行标题（最多 2 条 +N 场），无场次显示「休息」，今天绿描边、点击出浮层（全量场次/起止/时长/礼物）；卡片保持浅色，仅标签浅色卡配色对齐参考图；类型推断 `utils/liveType.ts`（9 类关键词，特殊=生日/周年/出道/庆典等） |
