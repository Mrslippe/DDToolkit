...
| └ 词云 | `.lc-dlg-cloud` + svg | 高 **210px**；`layoutPowerCloud(words, w, h)` = **Balzer 2005 加权 Voronoi（power diagram）**：线性半平面裁剪 + 站点弛豫至 power 单元质心 + λ += β(目标面积−当前面积)（β=0.5，120 轮上限，maxRel<0.1 早停）——**面积 ∝ 词频（数学保证），无边线（细胞共边）**；初始站点极坐标（hero=中心，黄金角），种子 mulberry32(20260907) 布局确定；**零外部依赖**（react-wordcloud/d3-delaunay 均已退役）；**破泡**（2026-09-07 user 定案）：点击单元 → 鼓泡缩灭（scale 1→1.12→0 + 淡出 0.42s）+ 质心环波（r 6→44 淡出 0.35s）→ 移除该词 → **挤入形变引擎**（见 B3.1）|
| └ 挤入形变 | `startMorph`（rAF 引擎） | 2026-09-07 user 定案「更贴近真实物理的挤过来填满」，**两次修正**：①形变插值替代平移滑位（凸多胞**质心等角度重采样 SAMPLE_N=28** 使新旧布局同构 → 逐帧插值 = 连续拉扯/压扁）；②**固定站点布局**（`fixedCellsFor`：破泡只删被破词的半平面约束，站点/λ 原地保留 → **邻居原位膨胀、远处纹丝不动**；初始全量弛豫仅一次作为「基座」`baseLayout`）——修复「全局重排（power-Lloyd）导致大幅位移」问题（node 实测：破掉第二大词，固定站点最大质心位移 78px vs 全局重排 386px，移动词数 5/11 vs 11/11，总面积=盒面积无洞）。波次推挤（距破泡质心 0–180ms 延迟梯度、近快远慢）+ 挤压呼吸（scale 1→1.07→1）+ easeOutBack(c1=0.9) 约 3% 过冲回弹；中途非凸深度 ≤13%（凸包比 ≥0.87）、**零自交**；形变期 `.morphing`（禁用 CSS 过渡）；尺寸变化/再破泡/恢复 → `cancelMorph` 落定终态；reduced-motion 直接终态 |
| └ 破泡恢复 | `.lc-dlg-sec-head` 内 `.lc-dlg-cloud-restore` | 「弹幕信息」段头行右侧胶囊（sel-bg 浅粉底粉字，同 fan-chart-reset 语言）：`已破泡 N · 恢复`（N>0 且数据非空才显示）；一键复原全部（restoreTick 信号）；切换场次自动清零；最后一个词破掉显示「已全部破泡（点击恢复还原）」 |
...
| └ 词字显示 | — | `fs = max(8.5, min(hero?34:26, r*0.75, r*2.2/len))`；`r>10.5 && len<=6 && fs>=8.5` 才显示（放宽后「哈哈哈哈」不再被吞）；**渲染结构**：每个单元 = 外层 `g.lc-dlg-cloud-cell`（`style transform: translate(cx,cy)` 定位，CSS 可过渡）+ 内层 `g.lc-dlg-cloud-bubble`（path/text 相对质心坐标，破泡缩放载体）；`cursor:pointer` |
