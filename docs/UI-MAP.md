...
| └ 挤入形变 | `startMorph`（rAF 引擎） | 2026-09-07 user 定案「更贴近真实物理的挤过来填满」：凸多胞**质心等角度重采样（SAMPLE_N=28）**使新旧布局同构（Voronei 单元是凸的）→ 逐帧插值 = 连续形变（拉扯/压扁）+ 质心轨迹 lerp + **波次推挤**（距破泡质心 0–180ms 延迟梯度、近快远慢）+ **挤压呼吸**（scale 1→1.07→1）+ easeOutBack(c1=0.9) 约 3% 过冲回弹；中途非凸深度 ≤13%（凸包比 ≥0.87）、**零自交**（node 原型实测）；形变期外层加 `.morphing`（禁用 CSS 过渡，与 JS 帧插值互斥）；尺寸变化/再破泡/恢复 → `cancelMorph` 落定终态；reduced-motion 直接终态 |
