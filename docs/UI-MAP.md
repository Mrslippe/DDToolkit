...
**四个数决定"内容从哪开始"**（R45 / R45-B / R45-D / **R45-E**，全仓唯一真源在 `tokens.css`）：
...
| `--toolbar-gap` | **51px** | **主体内容离它多远**（R45-D 定、**R45-E 改值**，见下） |

**⚠️ R45-E：让开量收归一处** —— `.view-body::before` 一条占位带，
`height: calc(var(--toolbar-band) + var(--toolbar-gap))`。**引用处只有这一个。**

> **为什么必须收归一处**（R45-E，用户 2026-09-24 拍板方案 A）：R45-B/D 时让开量写在
> 两个地方 —— `.page-title` 自己的 `padding-top`（list/archive）与 `.hero` 的 `padding-top`
> （cards）。两个问题：
> ① **profile 视图根本没有** ⇒ 四个视图的首块内容不在同一条线上，切换时内容会跳；
> ② 更根本的：`.hero` 的 `padding-top` 在**滚动容器内部** ⇒ **滚下去就没了**，
>    首块内容随即贴到工具条下面。
> 占位条在滚动容器**之外** ⇒ 首块内容永远从那一条线开始，滚下去由 `mask` 渐隐接管。

| 视图 | 让开由谁承担 | 页面标题 |
|---|---|---|
| cards | `.view-body::before` | 无（hero 本身就是主体） |
| list | 同上 | **有**（选中账号昵称） |
| archive | 同上 | **有**（当前卡片标题） |
| profile | 同上 | 无（用户 2026-09-24：「这轮不做 profile」） |

> **`--toolbar-gap` 为什么是 51px**（R45-E，用户 2026-09-24）：用户先给了两张参考图
> （R45-D，量出 **37px / 占面板高 7.0%** ⇒ 取 40px），随后**自己拼了一张合成图**把想要的
> 顶部版式摆出来给我对齐（R45-E）。后一张更具体、且是**按最终意图**拼的 ⇒ 以后者为准：
> 工具条下缘 y=50 → 账号行上缘 y=102 ⇒ **52px**，取 **51**。
...
| 性质 | R45-B/D（旧） | **R45-E（现）** |
|---|---|---|
| 定位 | `flex:none`，**占一行** | `position:absolute`，**不占流** |
| 与工具条 | 在它**下面**另起一行 | **同一栏**（垂直同轴：`top: calc(--toolbar-top + (--toolbar-h − 21.45)/2)`） |
| 让开量 | 写在标题的 `padding-top` 上 | 归 `.view-body::before`，**与标题无关** |
| 代价 | 白吃 **121px**（`92 + 21 + 8`） | **0**（内容位置与标题无关） |
| 文案 | list=账号昵称 / archive=卡片标题 / profile=无 | 同上 |

⚠️ **横向必须卡住"到工具条左缘有余量"**：条宽 176 居中 ⇒ 左缘恒在 `50% − 88px`
⇒ `max-width: calc(50% - 116px)`（= 条左缘 − `left:16` − 安全距 12）。
**1100 档最紧**（面板仅 558）：可用 163px，右缘 179 < 条左缘 191 ✓ —— 少减这 28px 就在窄档压上工具条。
⚠️ `.page-title` 是 `z-index:2`（**在工具条 3 之下**）、`pointer-events:none` —— 它只是占位物，
不该压过控件、也不该吃指针。
...
| 标题对账 | archive：导航标题 == 卡片内渲染的标题（两份真源，机器比） |
