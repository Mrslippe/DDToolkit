...
| └ 挤入形变 | `foamStep`（压力泡沫模拟，rAF） | 2026-09-07 user 定案「位置/大小/形状遵循物理规律、实时变化」（第三版）：**压力-弹簧泡沫模拟**（power diagram 物理原型）——每帧 ① 由(站点,λ)重算单元形状 ② 压力 p=k·(A−A_tgt)/A_tgt ③ 相邻泡对 F=(p_i−p_j)·L·n̂（共享弦长 L，顶点 0.1px 网格匹配）④ 墙力 ⑤ F/m 积分 + 阻尼 exp(−6·dt) ⑥ λ 面积调节（收敛后面积偏差 ≤1%）；**破泡 = 体积按邻边比例分给邻居 + 法向爆裂冲量(240) + 全簇 1/d 压力波(46)** → 大小/形状/位置全由压差涌现：邻居改大+被推（10-17px）、远处轻微、随阻尼果冻落定（~0.97s）；连续破泡从当前状态续算（连续性）；动能 < 阈值 24 帧自动停帧（静止即停，空闲零开销）；node 原型调参验证（模拟常数见 `LiveCalendar.tsx` SIM_* 注释）；reduced-motion 直接终态 |
