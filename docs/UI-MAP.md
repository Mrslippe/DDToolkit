...
| └ 词云 | `.lc-dlg-cloud` + svg | 高 **210px**；`layoutPowerCloud(words, w, h)` = **Balzer 2005 加权 Voronoi（power diagram）**：线性半平面裁剪 + 站点弛豫至 power 单元质心 + λ += β(目标面积−当前面积)（β=0.5，120 轮上限，maxRel<0.1 早停）——**面积 ∝ 词频（数学保证），无边线（细胞共边）**；初始站点极坐标（hero=中心，黄金角），种子 mulberry32(20260907) 布局确定；**零外部依赖**（react-wordcloud/d3-delaunay 均已退役）；**破泡**（2026-09-07 user 定案）：点击单元 → 鼓泡缩灭（scale 1→1.12→0 + 淡出 0.42s）+ 质心环波（r 6→44 淡出 0.35s）→ 移除该词重排 → 其余细胞经外层 g 的 `transform` transition 0.28s **滑向新质心**（结构调整：外层 g 定位 + 内层 `.lc-dlg-cloud-bubble` 承载缩放，`transform-box:fill-box` 居中）；破泡单元置顶渲染；hover/索引改以词 text 为键 |
