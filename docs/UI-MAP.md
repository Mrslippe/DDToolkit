...
| 档案设置窗口 | `.vd-settings*` | **P8-B 新增**：radix Dialog（`max-w-lg` + `max-height:78vh`）＝ 头部驻留（`.vd-settings-head`）＋ `OverlayScroll`（`.vd-settings-scroll`）＋ 底部操作条（`.vd-settings-foot`）；分区 `.vd-section`（背景/头像/签名/已订阅账号 —— 原「基本资料」2026-09-13 按用户口径整节删除，devlog/067 §四）、字段 `.vd-field`、账号行 `.vd-acc`。**全实时生效**（无保存钮：blur/点击即写库）；~~锁定胶囊 `.vd-lock.on`~~ **2026-09-13 退役**（devlog/074：`accounts.locked_fields` 已删，改为「允许覆盖 + 记曾用值」）。另有两处曾用值展示：签名区下方 `.vd-former`（「曾用签名：xxx（B站）· yyy（微博）」）、账号行下方 `.vd-acc-former`（「曾用名：…」，只在该账号确实改过名时出现）。规格遵循 UI-MAP §C6 |
| └ 签名 + 平台签名下拉 | `.vd-sign-field` / `.vd-sign-toggle` / `.vd-sign-panel` | 2026-09-13 两次迭代（devlog/072→073）：输入条右端**内嵌 chevron**（`.vd-sign-toggle`，22px 热区、右内距 30px、展开旋转 180°）→ 点开候选面板。面板是 **portal 到 body 的 `position:fixed` 浮层**（z-60，坐标由 JS 按输入条矩形算：同宽、贴下方 6px、放不下则向上翻转、跟随滚动/resize 重定位）—— 浮在「锁定/已订阅账号」之上、**不推挤**，也不会被 `.vd-settings-scroll` 的 `overflow:hidden` 裁掉；收起态不存在面板。候选行 `.vd-sign-opt`：**单行** = 签名文字 `.vd-sign-text`（弹性、`min-width:0`、横向可滚、滚动条隐藏）+ 右端固定平台名 `.vd-sign-plat`（主账号挂 `<em>主账号</em>`、当前项粉底白字）。文字过长时在平台名之前**渐隐**（**只在真的溢出时**挂 `.ovf` 的 `mask-image`）。交互（用户定）：**只留 hover 自动滚一次**到结尾、移出回起点；再点 chevron 收起（`mousedown` 的"点外部"判定把输入条也算内部）；`↑/↓/Enter/Esc` 键盘口径见组件注释。几何与结构不变量由 `ui_probe.py --settings` 断言（11 项）。**语义（A3，devlog/074）**：下拉里点某个平台 = 把卡片签名**来源**改成那个账号（写 `sign_source_account_id` 并**清掉 `sign_override`**），**不改任何 `accounts.sign`**；输入框打字 = 写 `sign_override`（覆盖优先于来源），清空输入框 = 撤销覆盖回到跟随来源；来源账号被删 → 回落主账号。有效值解析与卡片同口径（`utils/signSource.ts::resolveSign`） |
