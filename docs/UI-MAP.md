...
| └ 破泡 | `removeWord` + 重建 | 点击词 → **瞬间消失** → 用剩余词重建终态（入场路径，node 验证可靠：36 词偏差 15.5%/单调 100%/填满 99.7%）→ **400ms 同构插值**（凸多边形等角采样 28 点 + easeOutCubic lerp）——邻泡从当前位置平滑滑向新位置闭合空隙（"立即闭合"观感）；段头「已破泡 N · 恢复」胶囊一键还原（restoreTick → 重建初始布局） |
