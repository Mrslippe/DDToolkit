...
| **四角白边** | 4px 圆角的抗锯齿像素透出 **WebView 的白色背景** | 壳侧：`lib.rs::setup()` 的 `set_background_color(Color(0,0,0,0))` + `rebuild_main_window()` 的 `.background_color(...)` | R33/devlog135：`transparent(true)` 只管**窗口**层；**改 CSS 修不掉**；DWM 圆角/描边已排除（关掉后像素值不变） |
