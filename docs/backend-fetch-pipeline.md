...
- 下一轮到期 = `time.monotonic() + max(DYNAMICS_MIN_GAP_SECONDS(30s), 预算等待)
  ± DYNAMICS_JITTER_SECONDS(15s)`，**再套两层下限**：周期下限（R6：`max(该值, 轮开始 + 60s)`，
  见 §4.3.1.1）与 **R28 的两条**（见下）；`DYNAMICS_BUDGET_RPM<=0` 时退回固定
  `DYNAMICS_LATEST_INTERVAL_MINUTES=15min`；
- 动态流返回 `requests: {platform: n}` 便于观测；风控冷却（按平台、`600s` 起且**连续命中会升级**，
  R27/devlog/125）语义不变。
- **R28①：预算当"真上限"**（2026-09-16，devlog/127）：一轮装得下预算 → 不干预；装不下
  （`本轮需求 > rpm`）→ **下一轮至少等"这一轮消耗的预算时长"** = `(n / rpm) × 60s`。
  于是稳态速率回到 `rpm` 次/分钟，**不再随账号数线性上抬**（想更快就在设置里调高
  `DYNAMICS_BUDGET_RPM`）。历史（devlog/055）：逃逸口本意是"别让账号多的用户被自己饿死"——
  那一轮确实该跑，但**不该连稳态也跟着上抬**，R28 把它收成一次性。
- **R28②：闲下来就慢**：连续 **3 / 6 / 12** 轮没抓到新帖 ⇒ 轮间隔下限抬到
  **2 / 5 / 10 分钟**（封顶 10 分钟）；**恢复条件 = 抓到新帖 / 手动抓一次 / T0 检测到开播**。
  ⚠️ 开播检测走 T0（1 请求/分钟，不受影响）⇒ **与"推送要快"不冲突**（R25 推送时效的前提）。
- **R30：静默时段**（devlog/130）：用户指定的本地时段内（跨午夜也支持、`START == END` 不生效）
  轮间隔下限再抬到 `QUIET_HOURS_DYNAMICS_MIN_SECONDS`（默认 15 分钟），与 R28 档位取更保守者；
  **默认关闭**，且**只降动态流**（T0 不动，日历场次时间照旧准）。
- **预算按需自适应**（2026-09-13，devlog/055）：本平台生效预算 =
  ...
  取「一轮的需求量」为下限只消除退化；**R28① 起这个抬升只对"这一轮"生效**（见上），
  稳态仍由 `rpm` 决定。
...
**R35 起这段再加两半（devlog/139）—— 置顶帖不再只是"别绊倒增量停止"，本身也要维护**：

1. **落库 + 排序**：`pinned_ids` 从"单轮内存变量"变成库里的 `posts.is_pinned`；
...
> **冷却时长自 R27 起不再是固定 600s**（devlog/125）：`RATE_LIMIT_COOLDOWN` 只作为**基准**，
> 同一平台**连续命中**按 `1→base / 2→2×base / ≥3→4×base` 升级（封顶 60 分钟），
> 连续 6 小时无命中归零；冷却窗口**按平台落 `app_meta`**（重启与**应用内更新后的自动重启**
> 都不会遗忘），解禁后还有 10 分钟恢复期（动态流轮间隔 ×2）。下表里的"600s"都应读作"基准时长"。

| 层级 | 行为 |
|---|---|
| **列表页级**（视频页/动态页/微博页） | 风控 → 落盘已抓数据 → 同页冷却（基准 600s 起，连续命中会升级）重试，**上限 2 次**（`_PAGE_RETRIES`）；耗尽 → `stop_reason="rate_limited"`，前端提示 + 断点续抓（后续从该断点继续） |
...
1. **详情链节流**：0.5~2s → 2~3s（均匀随机）；拥塞窗口自适应**已完成一半**
   （R27，devlog/125：风控解禁后 10 分钟内动态流轮间隔翻倍 = 半预算），
   还剩"详情链"这一段没有按风控事件自适应；
2. **视频翻页 1s → 3s**；
