...
| `components/live/sessionGlance.ts`（R36） | 场次详情弹窗左列「本场速览」的四枚胶囊口径 | 纯函数 + 单测（组件渲染本仓测不了） | 0.5h | 低 | ✅ **已落地（2026-09-17，devlog/140）**：`glanceCapsules(session)` 恒返回四枚（时长 / 峰值在线 / 弹幕数 / 收益），值缺失一律 `—`（收益例外：`null` → `—`、`0` → `¥0`）；**6 条断言**钉"四枚恒定 + 顺序 + 空值口径"。渲染接线（卡片真在封面下方、2×2、高度零变化）由 `ui_probe.py --archive` 连采两格守 |
| `components/profile/`（R37-P1） | 档案视图 = 卡片画布：几何 / 注册表 / 两种内置卡片 | 新视图 + 4 个纯模块 | 6h | 中（"用户可编辑"的地基） | ✅ **P1 已落地（2026-09-17，devlog/141）**：`layoutModel.ts`（12 列网格几何，**17 条单测**）· `cardRegistry.ts`（扩展点，**4 条**）· `anniversary.ts`（宽容解析 + 倒计时，**14 条**）· `topPosts.ts`（优质投稿排序，**9 条**）· `ProfileBoardView.tsx`（视图）· `cards/{Anniversary,TopPosts}Card.tsx` + `cards/index.tsx`（注册点）。护栏：默认探针三档 `_assert_board`（高度=模型 / 不重叠 / 不越界 / **窄窗单列**）+ 卡片内容非空，**反向验证 5/5 红**。**P2 已落地**：P2a（devlog/142）迁移 `f006` + 表 + Repo + 两条路由 + purge 同步 + 15 条用例；P2b（devlog/144）读布局 / 编辑态 / 指针拖拽缩放（`moveCard`/`resizeCard` **推开**口径）/ 松手整版 PUT + 失败回滚，几何用例 **17 → 30**，探针 `--board` 端到端对账。**P4a 已落地（2026-09-18，devlog/146）**：卡片视觉规格化（**贴纸卡**：`--pcard-*` 令牌 + 贴纸角标；注册表新增 `icon`/`tone` 两个必填契约，用例 **4 → 7**）+ 三张卡重排（纪念日 hero **14 → 20** / 大事记时间线与 chip **10 → 14** / 优质投稿换 chip，口径未动、**12 条**不变），探针 `_assert_board` 加**材质 / 贴纸角标 / 内容不裁切**三组，**反向验证 42 红**；规格 `docs/design-archive-cards.md`（含动效时间轴与可断言不变量）。**P4b 已落地（2026-09-18，devlog/147）**：手势口径 **`motion.ts` 18 条单测**（相位机 / **跟手算式 `liftOffset`** / reduced 策略）+ `usePrefersReducedMotion` + **动效令牌**（`--motion-*` / `--ease-*` / `--motion-scale`，R38 批 1 复用）+ 长按拿起 / 跟手 1:1 / 落位收敛 + **动效调测页** `?motion=cards`；新探针 `--motion-cards`(±`--reduced`) 与 `--motion-lab`。⛔ P3b 自定义卡片（新增/删除 + `config_json`）· P4c 退避 FLIP（**路线 A：FLIP + 跟手偏移**，2026-09-18 拍板） |
