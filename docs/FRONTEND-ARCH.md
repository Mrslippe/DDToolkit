...
| `components/profile/`（R37-P1） | 档案视图 = 卡片画布：几何 / 注册表 / 两种内置卡片 | 新视图 + 4 个纯模块 | 6h | 中（"用户可编辑"的地基） | ✅ **P1 已落地（2026-09-17，devlog/141）**：`layoutModel.ts`（12 列网格几何，**17 条单测**）· `cardRegistry.ts`（扩展点，**4 条**）· `anniversary.ts`（宽容解析 + 倒计时，**14 条**）· `topPosts.ts`（优质投稿排序，**9 条**）· `ProfileBoardView.tsx`（视图）· `cards/{Anniversary,TopPosts}Card.tsx` + `cards/index.tsx`（注册点）。护栏：默认探针三档 `_assert_board`（高度=模型 / 不重叠 / 不越界 / **窄窗单列**）+ 卡片内容非空，**反向验证 5/5 红**。**P2 已落地**：P2a（devlog/142）迁移 `f006` + 表 + Repo + 两条路由 + purge 同步 + 15 条用例；P2b（devlog/144）读布局 / 编辑态 / 指针拖拽缩放（`moveCard`/`resizeCard` **推开**口径）/ 松手整版 PUT + 失败回滚，几何用例 **17 → 30**，探针 `--board` 端到端对账。⛔ P3 自定义卡片（新增/删除 + `config_json`）与扩展点接线 |
