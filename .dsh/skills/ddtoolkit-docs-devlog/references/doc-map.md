...
**2026-09-25 整节退役。** 这里原列 9 条"已知漂移"，逐条核对后**全部已修**；
连最后那条被标成"未决（用户明确暂缓）"的（R19/R20 在 `ROADMAP-DONE.md` 缺需求清单章节）
**其实更早就补上了** —— `ROADMAP-DONE.md` 的「需求清单：R19–R24」那一节（devlog/123）。

⇒ **一张不重新核对的"已知问题清单"比没有更糟**：读者会照着它去找已经不存在的问题，
而它自己永远不会红。**以后发现漂移就地修掉、或写进 devlog；不再维护"漂移清单"。**
（口径一律以源码 / 脚本 / `docs/README.md` 为准。）
